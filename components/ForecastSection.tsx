...
const ForecastSection: React.FC<{ data: ForecastAnalysis }> = ({ data }) => {
    ...
    const ProbPill: React.FC<{ prob: number }> = ({ prob }) => {
        const color = prob > 75 ? 'bg-green-500/20 text-green-500' : 'bg-yellow-500/20 text-yellow-500';
        return <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${color}`}>{prob.toFixed(0)}%</span>
    };

    return (
        <section className="space-y-8">
            <h2 className="text-xl font-bold text-text-main uppercase tracking-wider flex items-center gap-2">🔮 Previsão de Fechamentos</h2>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 bg-card border-border border rounded-xl p-6 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-2xl font-bold text-text-secondary opacity-70">{formatCurrency(data.next7Days.value.min)} - {formatCurrency(data.next7Days.value.max)}</p>
                ...
                <div className="lg:col-span-2 bg-card border-border border rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <p className="text-sm font-semibold mb-2 text-green-500">🔥 Alta Probabilidade ({'>'}75%)</p>
                            <ul className="text-sm space-y-2">{highProb.length > 0 ? highProb.map(l => (
                                <li key={l.name} className="flex justify-between items-center border border-border bg-bg-subtle/50 p-2 rounded-md">
                                    <div><p className="font-semibold text-text-main">{l.name}</p><p className="text-xs text-text-secondary">{l.status} • {formatCurrency(l.value)}</p></div>
                                    <ProbPill prob={l.probability} />
                                </li>
                            )) : <p className="text-xs text-text-secondary">Nenhum lead nesta categoria.</p>}</ul>
                        </div>
                        <div>
                            <p className="text-sm font-semibold mb-2 text-yellow-500">🟡 Média Probabilidade (50-75%)</p>
